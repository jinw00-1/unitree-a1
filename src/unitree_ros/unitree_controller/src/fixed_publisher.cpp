...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "fixed_publisher");
    ros::NodeHandle nh;
    ros::Publisher fixed_publisher = nh.advertise<gazebo_msgs::ModelState>("/gazebo/set_model_state", 1000);
    ...
    if(def_frame == coord::WORLD)
    {
        ...
        model_state_pub.pose.position.z = 1.0;
        
        model_state_pub.pose.orientation.x = 0.0;
        model_state_pub.pose.orientation.y = 0.3;
        ...
        while(ros::ok())
        {
            ...
            fixed_publisher.publish(model_state_pub);
            loop_rate.sleep();
            time_ms += 1;
        }
    }
